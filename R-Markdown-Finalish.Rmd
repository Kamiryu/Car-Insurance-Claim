...
# Histograms
cars %>%
  select(1:3, 5:6, 13, 20:21, 23, 25:29, 42:43) %>%
  gather() %>%
  ggplot(aes(value)) +
...
# Barcharts
cars %>%
  select(4, 7:12, 14:19, 22, 24, 30:41) %>%
  gather() %>%
  ggplot(aes(x = value)) +
  facet_wrap(~key, scales = "free") + # Display figures in many facets
...
cars_reduced_rf <- cars_reduced[, c(
...
corrplot(as.matrix(cor3),
  add = TRUE, # Plot of upper right part
  type = "upper",
  method = "ellipse",
  order = "AOE",
  diag = FALSE,
...
cp_minimal <- tree_fit$cptable[
  which.min(
    tree_fit$cptable[, "CP"]
  ), # Take lowest cp and Take lowest xerror
  1
] # CP column
...
train_dummy_downsampled <- downSample(train_dummy[, -16],
  train_dummy$is_claim,
  yname = "is_claim"
) # undersampling for having same probation
...
for (i in 1:50) {
  ...
  Specificity[i, 2] <- confusionMatrix(
    knn.pred,
    valid_dummy[, 16]
  )$byClass[2]
}
...
for (i in 1:50) {
  ...
  Sensitivity[i, 2] <- confusionMatrix(
    knn.pred,
    valid_dummy[, 16]
  )$byClass[1]
}
...
ggplot(accuracy, aes(k, overallaccuracy)) +
  ...
  theme_minimal() +
  labs(
    title = "K nearest neighbours: Overall accuracy vs K",
    y = "Accuracy",
...
ggplot(Sensitivity, aes(k, overallSensitivity)) +
  ...
  theme_minimal() +
  labs(
    title = "K nearest neighbours: Overall Sensitivity vs K",
    y = "Sensitivity",
    x = "k nearest neigbours"
  )
...
ggplot(Specificity, aes(k, overallspecificity)) +
  ...
  theme_minimal() +
  labs(
    title = "K nearest neighbours: Overall specificity vs K",
    y = "Specificity",
    x = "k nearest neigbours"
  )
```
