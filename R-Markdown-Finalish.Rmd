...
To conduct our research, we plan to undertake the following steps. First, we will explore the data. [Add STEPS] [Add STEPS] [Add STEPS] [Add STEPS]

## [Add STEPS] To be continued
...
# Normalizing policy tenure
 normalized.policy <- (train_downsampling$policy_tenure - min(train_downsampling$policy_tenure)) / (max (train_downsampling$policy_tenure)-min  (train_downsampling$policy_tenure))
train_downsampling$policy_tenure <- as.vector(normalized.policy)
  
#   Normalizing age_of car
  normalized.age <- (train_downsampling$age_of_car - min  (train_downsampling$age_of_car)) / (max (train_downsampling$age_of_car)-min (train_downsampling$age_of_car))
train_downsampling$age_of_car <- as.vector(normalized.age)
 
  
#  Normalizing age of policy holder
 normalized.age.policy <- (train_downsampling$age_of_policyholder - min
                           (train_downsampling$age_of_policyholder)) / (max (train_downsampling$age_of_policyholder)-min  (train_downsampling$age_of_policyholder))
train_downsampling$age_of_policyholder <- as.vector(normalized.age.policy)
summary(train_downsampling)
```
...
The resulting pruned tree with at its best size has 15 leaves.
...
Since we are looking fo k which give us max optimum value of Specificity we chose k = 46

```{r}
knn.pred.valid <- knn(train_new[, -16], valid_dummy[, -16], cl = train_new[, 16], k = 46 ,prob = T)     #prediction on validation data using best k=6
...
With a top decile lift of 1.51, the model's lift drastically decreases at the second decile then slowly declines with each decile.
...
```{r}
Accuracy <-  data.frame(LR = cm2$overall[1])  #predict accuracy by using logistic regression 
Accuracy <- cbind(Accuracy, KNN = cmk$overall[1])      #predict accuracy by using KNN
Accuracy <- cbind(Accuracy,CT = conf_matrix_tree_v$overall[1])    #predict accuracy by using tree
Accuracy <- cbind(Accuracy,NN=conf_matrix_1H3N_valid$overall[1])     #predict accuracy by using neural network
...
```{r}
Specificity_P <-  data.frame(LR = cm2$byClass[2])  #predict specificity by using logistic regression 
Specificity_P <- cbind(Specificity_P, KNN = cmk$byClass[2])      #predict accuracy by using KNN
Specificity_P <- cbind(Specificity_P,CT = conf_matrix_tree_v$byClass[2])    #predicts pecificity by using tree
Specificity_P <- cbind(Specificity_P,NN=conf_matrix_1H3N_valid$byClass[2])     #predict specificity by using neural network
